...
std::vector<TestParam> generateTestParams() {
  ...
  std::string expectedOutput;
  if (!inputFile) {
    std::cerr << "Unable to open test inputs file\n";
    return params;
  }
  while (std::getline(inputFile, line)) {
    if (line == testScenarioSeparator) {
      std::string outputLine;
      while (std::getline(expectedOutputFile, outputLine) &&
             outputLine != testScenarioSeparator) {
...
INSTANTIATE_TEST_SUITE_P(PbTest, PbTest,
